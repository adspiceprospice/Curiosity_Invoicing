...
export default function OffersList({ offers, customers, statusFilter, customerFilter }: OffersListProps) {
  ...
  const filteredOffers = offers.filter(offer => {
    ...
    const searchMatch = searchTerm.trim() === '' || 
      offer.documentNumber.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <>
      <Head>
        <title>{t('offers.title')} | Curiosity Invoicing</title>
      </Head>

      <DashboardLayout>
        <div className="mb-6 flex justify-between items-center">
          <h1 className="text-2xl font-semibold text-gray-900">{t('offers.title')}</h1>
          <Link
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
            <div className="flex flex-col md:flex-row md:items-center space-y-4 md:space-y-0 md:space-x-4">
              ...
              <div className="w-full md:w-64">
                ...
                >
                  <option value="">{t('offers.allCustomers')}</option>
            ...
            <div className="flex flex-wrap gap-2">
              {['DRAFT', 'SENT', 'ACCEPTED', 'DECLINED', 'EXPIRED', 'VOIDED'].map((status) => (
                <button
                  key={status}
                  type="button"
                ...
                >
                  {t(`offers.status.${status.toLowerCase()}`)}
                </button>
              ))}
            </div>
          </div>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center space-x-4">
                          ...
                          <div className="text-sm font-medium text-gray-900">
                            {formatCurrency(parseFloat(offer.totalAmount.toString()))}
                          </div>
                          <div className="text-sm text-gray-500 hidden md:block">
                            {formatDate(offer.issueDate)}
                          </div>
                          <div className="text-sm text-gray-500 hidden lg:block">
                            {offer._count.lineItems} {offer._count.lineItems === 1 ? 'item' : 'items'}
                          </div>
                          <svg className="h-5 w-5 text-gray-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
            ...
            <div className="flex">
              ...
              <div className="ml-3 flex-1 md:flex md:justify-between">
                ...
                <p className="mt-3 text-sm md:mt-0 md:ml-6">
                  <Link href="/dashboard/offers/new" className="whitespace-nowrap font-medium text-blue-700 hover:text-blue-600">
                    {t('offers.createFirstOffer')} <span aria-hidden="true">&rarr;</span>
                  </Link>
                </p>
              </div>
            </div>
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  try {
    ...
    return {
      props: {
        offers: JSON.parse(JSON.stringify(offers)), // Handle serialization of dates
