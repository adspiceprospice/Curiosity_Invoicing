...
export default async function handler(
  ...
  try {
    ...
    let pdfUrl = `/api/invoices/${id}/pdf`; // Default URL for local access
    
    try {
      // Try to upload to Google Drive if integration is enabled
      const driveResult = await uploadDocumentToDrive(req, id as string, pdfBuffer, 'invoice');
    ...
    // Update the invoice with the PDF URL (either Google Drive or local)
